...
def get_results(
    ...
    fmodel, num, dr, hidden, lr, wd = U.best_model_params(
        study_name, storage, subfields_train, root_files, label_train, "new", sim_train, monopole_train
    )
    ...
    if not (monopole_train):
        suffix1 = "%s_no_monopole" % suffix1
    suffix2 = "test_%s%s_%s" % (sim_test, suffix_test, label_test)
    if not (monopole_test):
        suffix2 = "%s_no_monopole" % suffix2
...
if smoothing_train > 0:
    label_train = "%s_smoothing_%d" % (label_train, smoothing_train)
if smoothing_test > 0:
    label_test = "%s_smoothing_%d" % (label_test, smoothing_train)
...
if suite == "LH":
    ...
    raise Exception("wrong suite value")
...
for field_train, field_test in zip(fields_train, fields_test):
    ...
    if field_train == "Comb":
        subfields_train = ["Mgas", "Mtot", "Mstar", "Vgas", "T", "Z", "P", "HI", "ne", "B", "MgFe"]
    else:
        subfields_train = [field_train]

    # get the suffix
    suffix_train = ""
    for x in subfields_train:
        suffix_train = "%s_%s" % (suffix_train, x)

    # get the storage name to use best-model (training)
    storage = U.fname_storage(root_storage, subfields_train, label_train, monopole=monopole_train)
    #########################################

    ########## Testing files ################
    # get the subfields of the considered field
    if field_test == "Comb":
        subfields_test = ["Mgas", "Mtot", "Mstar", "Vgas", "T", "Z", "P", "HI", "ne", "B", "MgFe"]
    else:
        subfields_test = [field_test]

    # get the name of the maps to test the model
    channels = len(subfields_test)
    f_maps, f_maps_norm, suffix_test = U.fname_maps(root_maps, subfields_test, sim_test, z, suite, sim_norm)

    # get the test data
    test_loader = data.create_dataset_multifield(
        mode,
        seed,
        f_maps,
        f_params,
        ...
        splits,
        f_maps_norm,
        monopole=monopole_test,
        monopole_norm=monopole_train,
        rot_flip_in_mem=True,
        shuffle=False,
        just_monopole=just_monopole,
        smoothing=smoothing_test,
        smoothing_norm=smoothing_train,
        verbose=True,
    )
    #########################################
