...
class Objective(object):
    ...
    def __call__(self, trial):
        ...
        if self.monopole:
            suffix = "%d" % trial.number
        else:
            suffix = "no_monopole_%d" % trial.number
        if self.label is not None:
            suffix = "%s_%s" % (suffix, label)
        ...
        print("\nPreparing datasets...")
        train_loader = data.create_dataset_multifield(
            "train",
            self.seed,
            self.f_maps,
            self.f_params,
            ...
            rot_flip_in_mem=self.rot_flip_in_mem,
            smoothing=self.smoothing,
            verbose=True,
        )
        valid_loader = data.create_dataset_multifield(
            "valid",
            self.seed,
            self.f_maps,
            self.f_params,
        ...
        model = architecture.get_architecture(
            self.arch + "_err", hidden, dr, channels, pretrained=self.resnet_pretrained
        )
        ...
        optimizer = torch.optim.AdamW(model.parameters(), lr=max_lr, weight_decay=wd, betas=(self.beta1, self.beta2))
        scheduler = torch.optim.lr_scheduler.CyclicLR(
            optimizer, base_lr=self.min_lr, max_lr=max_lr, cycle_momentum=False, step_size_up=500, step_size_down=500
        )
        ...
        for epoch in range(self.epochs):
            ...
            # verbose
            print("%03d %.3e %.3e %.3e " % (epoch, train_loss, valid_loss, test_loss), end="")
...
param_dump = {
    ...
    if k in d and not k.startswith("__") and not isinstance(v, Type) and v is not Type and not isinstance(v, ModuleType)
}

param_dump_file = (
    U.fname_storage(root_storage, fields, label, monopole=monopole).removeprefix("sqlite:///").removesuffix(".db")
    + ".json"
)
