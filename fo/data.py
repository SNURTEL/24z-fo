...
def smooth_maps(maps, smoothing, verbose=True):
    if verbose:
        print("Smoothing images with smoothing length: %d" % smoothing)
...
class make_dataset_multifield2_errors:
    def __init__(
        ...
        fmodel_m, num, dr, hidden, lr, wd = U.best_model_params(study_name, storage_m, fields, root_files, label_m)
        ...
        if os.path.exists(fmodel_m):
            print("Loading model...")
            model_m.load_state_dict(torch.load(fmodel_m, map_location=torch.device(device)))
        else:
...
class make_dataset_multifield2:
    def __init__(
        ...
        for channel, (fim, fnorm) in enumerate(zip(f_images, f_images_norm)):
            ...
            # smooth the images
            if smoothing > 0:
                data_c = smooth_maps(data_c, smoothing, verbose)
...
class make_dataset_multifield:
    def __init__(
        ...
        for channel, (fim, fnorm) in enumerate(zip(f_images, f_images_norm)):
            ...
            # smooth the images
            if smoothing > 0:
                data_c = smooth_maps(data_c, smoothing)
...
class make_dataset:
    def __init__(self, mode, seed, f_images, f_params, splits, f_images_norm, monopole, verbose):
        # read the data
        ...
        # normalize maps
        if verbose:
            print("%.3e < T(all|orig) < %.3e" % (np.min(data), np.max(data)))
        if f_images.find("Mstar") != -1:
        ...
        if f_images_norm is None:
            ...
            del data_norm
        if verbose:
            print("%.3f < T(all|log)  < %.3f" % (np.min(data), np.max(data)))
        # data = 2*(data - minimum)/(maximum - minimum) - 1.0
        data = (data - mean) / std
        if verbose:
            print("%.3f < T(all|norm) < %.3f" % (np.min(data), np.max(data)))
        ...
        data_all = np.zeros((size_maps * 8, data.shape[1], data.shape[2]), dtype=np.float32)
...
def create_dataset(
    mode, seed, f_images, f_params, batch_size, splits, f_images_norm=None, monopole=True, verbose=False
):
    data_set = make_dataset(mode, seed, f_images, f_params, splits, f_images_norm, monopole, verbose)
...
def create_dataset_multifield(
    ...
    if rot_flip_in_mem:
        data_set = make_dataset_multifield(
            mode,
            seed,
            f_images,
            f_params,
        ...
        data_set = make_dataset_multifield2(
            mode,
            seed,
            f_images,
            f_params,
    ...
    data_loader = DataLoader(dataset=data_set, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers)
    return data_loader


# This routine returns the maps, their parameters and their errors. Not really used
def create_dataset_multifield2_errors(
    mode,
    seed,
    f_images,
    f_params,
    ...
    data_set = make_dataset_multifield2_errors(
        mode,
        seed,
        f_images,
        f_params,
