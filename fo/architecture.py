...
def get_architecture(arch, hidden, dr, channels=1, pretrained=True):
    if arch == "a":
        return model_a(hidden)
    elif arch == "b":
        return model_b(hidden)
    elif arch == "c":
        return model_c(hidden)
    elif arch == "d":
        return model_d(hidden)
    elif arch == "e":
        return model_e(hidden, dr)
    elif arch == "e2":
        return model_e2(hidden, dr)
    elif arch == "e3":
        return model_e3(hidden, dr, channels)
    elif arch == "e3_err":
        return model_e3_err(hidden, dr, channels)
    elif arch == "e3_abs":
        return model_e3_abs(hidden, dr, channels)
    elif arch == "e3_res":
        return model_e3_res(hidden, dr)
    elif arch == "f":
        return model_f(hidden, dr)
    elif arch == "f2":
        return model_f2(hidden, dr)
    elif arch == "f3":
        return model_f3(hidden, dr)
    elif arch == "g":
        return model_g(hidden, dr)
    elif arch == "g2":
        return model_g2(hidden, dr)
    elif arch == "h3_err":
        return model_h3_err(hidden, dr, channels)
    elif arch == "i3_err":
        return model_i3_err(hidden, dr, channels)
    elif arch == "j3_err":
        return model_j3_err(hidden, dr, channels)
    elif arch == "k3_err":
        return model_k3_err(hidden, dr, channels)
    elif arch == "l3_err":
        return model_l3_err(hidden, dr, channels)
    elif arch == "m3_err":
        return model_m3_err(hidden, dr, channels)
    elif arch == "n3_err":
        return model_n3_err(hidden, dr, channels)
    elif arch == "o3_err":
        return model_o3_err(hidden, dr, channels)
    elif arch == "resnet18_err":
        return resnet_18(dr=dr, pretrained=pretrained)
    else:
        raise Exception("Architecture %s not found" % arch)
...
class model_a(nn.Module):
    def __init__(self, hidden):
        ...
        self.C1 = nn.Conv2d(1, hidden, kernel_size=4, stride=2, padding=1, bias=True)
        self.B1 = nn.BatchNorm2d(hidden)
        # input: hiddenx125x125 ----------> output: 2*hiddenx62x62
        self.C2 = nn.Conv2d(hidden, 2 * hidden, kernel_size=5, stride=2, padding=1, bias=True)
        self.B2 = nn.BatchNorm2d(2 * hidden)
        # input: 2*hiddenx62x62 --------> output: 4*hiddenx31x31
        self.C3 = nn.Conv2d(2 * hidden, 4 * hidden, kernel_size=4, stride=2, padding=1, bias=True)
        self.B3 = nn.BatchNorm2d(4 * hidden)
        # input: 4*hiddenx31x31 ----------> output: 8*hiddenx15x15
        self.C4 = nn.Conv2d(4 * hidden, 8 * hidden, kernel_size=5, stride=2, padding=1, bias=True)
        self.B4 = nn.BatchNorm2d(8 * hidden)
        # input: 8*hiddenx15x15 ----------> output: 16*hiddenx7x7
        self.C5 = nn.Conv2d(8 * hidden, 16 * hidden, kernel_size=5, stride=2, padding=1, bias=True)
        self.B5 = nn.BatchNorm2d(16 * hidden)
        # input: 16*hiddenx7x7 ----------> output: 100x3x3
        self.C6 = nn.Conv2d(16 * hidden, 100, kernel_size=5, stride=2, padding=1, bias=True)
...
class model_b(nn.Module):
    def __init__(self, hidden):
        ...
        self.C2 = nn.Conv2d(hidden, hidden, kernel_size=5, stride=2, padding=1, bias=True)
        self.B2 = nn.BatchNorm2d(hidden)
        # input: hiddenx62x62 --------> output: 2*hiddenx31x31
        self.C3 = nn.Conv2d(hidden, 2 * hidden, kernel_size=4, stride=2, padding=1, bias=True)
        self.B3 = nn.BatchNorm2d(2 * hidden)
        # input: 2*hiddenx31x31 ----------> output: 2*hiddenx15x15
        self.C4 = nn.Conv2d(2 * hidden, 2 * hidden, kernel_size=5, stride=2, padding=1, bias=True)
        self.B4 = nn.BatchNorm2d(2 * hidden)
        # input: 2*hiddenx15x15 ----------> output: 4*hiddenx7x7
        self.C5 = nn.Conv2d(2 * hidden, 4 * hidden, kernel_size=5, stride=2, padding=1, bias=True)
        self.B5 = nn.BatchNorm2d(4 * hidden)
        # input: 4*hiddenx7x7 ----------> output: 100x3x3
        self.C6 = nn.Conv2d(4 * hidden, 100, kernel_size=5, stride=2, padding=1, bias=True)
        self.B6 = nn.BatchNorm2d(100)
...
class model_c(nn.Module):
    def __init__(self, hidden):
        ...
        # input: 1x250x250 ---------------> output: hiddenx125x125
        self.C1 = nn.Conv2d(1, hidden, kernel_size=4, stride=2, padding=1, padding_mode="circular", bias=True)
        self.B1 = nn.BatchNorm2d(hidden)
        # input: hiddenx125x125 ----------> output: hiddenx62x62
        self.C2 = nn.Conv2d(hidden, 2 * hidden, kernel_size=5, stride=2, padding=1, padding_mode="circular", bias=True)
        self.B2 = nn.BatchNorm2d(2 * hidden)
        # input: hiddenx62x62 --------> output: 2*hiddenx31x31
        self.C3 = nn.Conv2d(
            2 * hidden, 4 * hidden, kernel_size=4, stride=2, padding=1, padding_mode="circular", bias=True
        )
        self.B3 = nn.BatchNorm2d(4 * hidden)
        # input: 2*hiddenx31x31 ----------> output: 2*hiddenx15x15
        self.C4 = nn.Conv2d(
            4 * hidden, 8 * hidden, kernel_size=5, stride=2, padding=1, padding_mode="circular", bias=True
        )
        self.B4 = nn.BatchNorm2d(8 * hidden)
        # input: 2*hiddenx15x15 ----------> output: 4*hiddenx7x7
        self.C5 = nn.Conv2d(
            8 * hidden, 16 * hidden, kernel_size=5, stride=2, padding=1, padding_mode="circular", bias=True
        )
        self.B5 = nn.BatchNorm2d(16 * hidden)
        # input: 4*hiddenx7x7 ----------> output: 100x3x3
        self.C6 = nn.Conv2d(16 * hidden, 100, kernel_size=5, stride=2, padding=1, padding_mode="circular", bias=True)
        self.B6 = nn.BatchNorm2d(100)
...
class model_d(nn.Module):
    def __init__(self, hidden):
        ...
        # input: 4*hiddenx7x7 ----------> output: 100x3x3
        self.C6 = nn.Conv2d(
            16 * hidden, 32 * hidden, kernel_size=5, stride=2, padding=1, padding_mode="circular", bias=True
        )
...
class model_e2(nn.Module):
    def __init__(self, hidden, dr):
        ...
        # input: 1x250x250 ---------------> output: hiddenx125x125
        self.C1 = nn.Conv2d(1, hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True)
        self.C2 = nn.Conv2d(hidden, hidden, kernel_size=4, stride=2, padding=1, padding_mode="circular", bias=True)
        self.B1 = nn.BatchNorm2d(hidden)
        self.B2 = nn.BatchNorm2d(hidden)

        # input: hiddenx125x125 ----------> output: hiddenx62x62
        self.C3 = nn.Conv2d(hidden, 2 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True)
        self.C4 = nn.Conv2d(
            2 * hidden, 2 * hidden, kernel_size=5, stride=2, padding=1, padding_mode="circular", bias=True
        )
        self.B3 = nn.BatchNorm2d(2 * hidden)
        self.B4 = nn.BatchNorm2d(2 * hidden)
        ...
        # input: hiddenx62x62 --------> output: 2*hiddenx31x31
        self.C5 = nn.Conv2d(
            2 * hidden, 4 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        self.C6 = nn.Conv2d(
            4 * hidden, 4 * hidden, kernel_size=4, stride=2, padding=1, padding_mode="circular", bias=True
        )
        self.B5 = nn.BatchNorm2d(4 * hidden)
        ...
        # input: 2*hiddenx31x31 ----------> output: 2*hiddenx15x15
        self.C7 = nn.Conv2d(
            4 * hidden, 8 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        self.C8 = nn.Conv2d(
            8 * hidden, 8 * hidden, kernel_size=5, stride=2, padding=1, padding_mode="circular", bias=True
        )
        ...
        # input: 2*hiddenx15x15 ----------> output: 4*hiddenx7x7
        self.C9 = nn.Conv2d(
            8 * hidden, 16 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        self.C10 = nn.Conv2d(
            16 * hidden, 16 * hidden, kernel_size=5, stride=2, padding=1, padding_mode="circular", bias=True
        )
        ...
        # input: 4*hiddenx7x7 ----------> output: 100x3x3
        self.C11 = nn.Conv2d(
            16 * hidden, 32 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        self.C12 = nn.Conv2d(
            32 * hidden, 32 * hidden, kernel_size=5, stride=2, padding=1, padding_mode="circular", bias=True
        )
...
class model_e3(nn.Module):
    def __init__(self, hidden, dr, channels):
        ...
        # input: 1x250x250 ---------------> output: hiddenx125x125
        self.C01 = nn.Conv2d(channels, hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True)
        self.C02 = nn.Conv2d(hidden, hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True)
        self.C03 = nn.Conv2d(hidden, hidden, kernel_size=4, stride=2, padding=1, padding_mode="circular", bias=True)
        self.B01 = nn.BatchNorm2d(hidden)
        self.B02 = nn.BatchNorm2d(hidden)
        self.B03 = nn.BatchNorm2d(hidden)

        # input: hiddenx125x125 ----------> output: hiddenx62x62
        self.C11 = nn.Conv2d(hidden, 2 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True)
        self.C12 = nn.Conv2d(
            2 * hidden, 2 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        self.C13 = nn.Conv2d(
            2 * hidden, 2 * hidden, kernel_size=5, stride=2, padding=1, padding_mode="circular", bias=True
        )
        ...
        self.C22 = nn.Conv2d(
            4 * hidden, 4 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        self.C23 = nn.Conv2d(
            4 * hidden, 4 * hidden, kernel_size=4, stride=2, padding=1, padding_mode="circular", bias=True
        )
        ...
        self.C32 = nn.Conv2d(
            8 * hidden, 8 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        self.C33 = nn.Conv2d(
            8 * hidden, 8 * hidden, kernel_size=5, stride=2, padding=1, padding_mode="circular", bias=True
        )
        ...
        self.C42 = nn.Conv2d(
            16 * hidden, 16 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        self.C43 = nn.Conv2d(
            16 * hidden, 16 * hidden, kernel_size=5, stride=2, padding=1, padding_mode="circular", bias=True
        )
        ...
        self.C52 = nn.Conv2d(
            32 * hidden, 32 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        self.C53 = nn.Conv2d(
            32 * hidden, 32 * hidden, kernel_size=5, stride=2, padding=1, padding_mode="circular", bias=True
        )
...
class model_e3_res(nn.Module):
    def __init__(self, hidden, dr):
        ...
        # input: 1x250x250 ---------------> output: hiddenx125x125
        self.C01 = nn.Conv2d(1, hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True)
        self.C02 = nn.Conv2d(hidden, hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True)
        self.C03 = nn.Conv2d(hidden, hidden, kernel_size=4, stride=2, padding=1, padding_mode="circular", bias=True)
        self.B01 = nn.BatchNorm2d(hidden)
        self.B02 = nn.BatchNorm2d(hidden)
...
class model_j3_err(nn.Module):
    def __init__(self, hidden, dr, channels):
        ...
        self.C61 = nn.Conv2d(
            32 * hidden, 64 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        self.C62 = nn.Conv2d(
            64 * hidden, 64 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        self.C63 = nn.Conv2d(
            64 * hidden, 64 * hidden, kernel_size=5, stride=1, padding=1, padding_mode="circular", bias=True
        )
...
class model_k3_err(nn.Module):
    def __init__(self, hidden, dr, channels):
        ...
        self.C00 = nn.Conv2d(channels, hidden, kernel_size=1, stride=1, padding=0, padding_mode="circular", bias=True)
        self.C01 = nn.Conv2d(channels, hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True)
        self.C02 = nn.Conv2d(hidden, hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True)
        self.C03 = nn.Conv2d(hidden, hidden, kernel_size=2, stride=2, padding=0, padding_mode="circular", bias=True)
        self.B01 = nn.BatchNorm2d(hidden)
        self.B02 = nn.BatchNorm2d(hidden)
        self.B03 = nn.BatchNorm2d(hidden)

        # input: hiddenx128x128 ----------> output: 2*hiddenx64x64
        self.C10 = nn.Conv2d(hidden, 2 * hidden, kernel_size=1, stride=1, padding=0, padding_mode="circular", bias=True)
        self.C11 = nn.Conv2d(hidden, 2 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True)
        self.C12 = nn.Conv2d(
            2 * hidden, 2 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        self.C13 = nn.Conv2d(
            2 * hidden, 2 * hidden, kernel_size=2, stride=2, padding=0, padding_mode="circular", bias=True
        )
        self.B11 = nn.BatchNorm2d(2 * hidden)
        self.B12 = nn.BatchNorm2d(2 * hidden)
        self.B13 = nn.BatchNorm2d(2 * hidden)
        ...
        self.C20 = nn.Conv2d(
            2 * hidden, 4 * hidden, kernel_size=1, stride=1, padding=0, padding_mode="circular", bias=True
        )
        self.C21 = nn.Conv2d(
            2 * hidden, 4 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        ...
        self.C23 = nn.Conv2d(
            4 * hidden, 4 * hidden, kernel_size=2, stride=2, padding=0, padding_mode="circular", bias=True
        )
        self.B21 = nn.BatchNorm2d(4 * hidden)
        self.B22 = nn.BatchNorm2d(4 * hidden)
        self.B23 = nn.BatchNorm2d(4 * hidden)
        ...
        self.C30 = nn.Conv2d(
            4 * hidden, 8 * hidden, kernel_size=1, stride=1, padding=0, padding_mode="circular", bias=True
        )
        self.C31 = nn.Conv2d(
            4 * hidden, 8 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        ...
        self.C33 = nn.Conv2d(
            8 * hidden, 8 * hidden, kernel_size=2, stride=2, padding=0, padding_mode="circular", bias=True
        )
        self.B31 = nn.BatchNorm2d(8 * hidden)
        self.B32 = nn.BatchNorm2d(8 * hidden)
        self.B33 = nn.BatchNorm2d(8 * hidden)
        ...
        self.C40 = nn.Conv2d(
            8 * hidden, 16 * hidden, kernel_size=1, stride=1, padding=0, padding_mode="circular", bias=True
        )
        self.C41 = nn.Conv2d(
            8 * hidden, 16 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        ...
        self.C43 = nn.Conv2d(
            16 * hidden, 16 * hidden, kernel_size=2, stride=2, padding=0, padding_mode="circular", bias=True
        )
        self.B41 = nn.BatchNorm2d(16 * hidden)
        self.B42 = nn.BatchNorm2d(16 * hidden)
        self.B43 = nn.BatchNorm2d(16 * hidden)
        ...
        self.C50 = nn.Conv2d(
            16 * hidden, 32 * hidden, kernel_size=1, stride=1, padding=0, padding_mode="circular", bias=True
        )
        self.C51 = nn.Conv2d(
            16 * hidden, 32 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        ...
        self.C53 = nn.Conv2d(
            32 * hidden, 32 * hidden, kernel_size=2, stride=2, padding=0, padding_mode="circular", bias=True
        )
        self.B51 = nn.BatchNorm2d(32 * hidden)
        self.B52 = nn.BatchNorm2d(32 * hidden)
        self.B53 = nn.BatchNorm2d(32 * hidden)
        ...
        self.C60 = nn.Conv2d(
            32 * hidden, 64 * hidden, kernel_size=1, stride=1, padding=0, padding_mode="circular", bias=True
        )
        self.C61 = nn.Conv2d(
            32 * hidden, 64 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        ...
        self.C63 = nn.Conv2d(
            64 * hidden, 64 * hidden, kernel_size=4, stride=1, padding=0, padding_mode="circular", bias=True
        )
        self.B61 = nn.BatchNorm2d(64 * hidden)
        self.B62 = nn.BatchNorm2d(64 * hidden)
        self.B63 = nn.BatchNorm2d(64 * hidden)
...
class model_l3_err(nn.Module):
    def __init__(self, hidden, dr, channels):
        ...
        self.C61 = nn.Conv2d(
            32 * hidden, 150 * hidden, kernel_size=4, stride=1, padding=0, padding_mode="circular", bias=True
        )
...
class model_m3_err(nn.Module):
    def __init__(self, hidden, dr, channels):
        ...
        self.C00 = nn.Conv2d(
            channels, 2 * hidden, kernel_size=1, stride=1, padding=0, padding_mode="circular", bias=True
        )
        self.C01 = nn.Conv2d(
            channels, 2 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        self.C02 = nn.Conv2d(
            2 * hidden, 2 * hidden, kernel_size=3, stride=1, padding=1, padding_mode="circular", bias=True
        )
        ...
        self.C53 = nn.Conv2d(
            64 * hidden, 64 * hidden, kernel_size=2, stride=2, padding=0, padding_mode="circular", bias=True
        )
        ...
        self.C61 = nn.Conv2d(
            64 * hidden, 128 * hidden, kernel_size=4, stride=1, padding=0, padding_mode="circular", bias=True
        )
